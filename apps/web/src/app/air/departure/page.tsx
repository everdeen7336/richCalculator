...
function getDepartureGateLabel(terminal: Terminal, gateKey: string): string {
  if (terminal === Terminal.T2) {
    const t2Map: Record<string, string> = { gate1: '출국장 1', gate2: '출국장 2', gate3: '출국장', gate4: '-', gate56: '-' };
    return t2Map[gateKey] || gateKey;
  }
  const t1Map: Record<string, string> = { gate1: '1번', gate2: '2번', gate3: '3번', gate4: '4번', gate56: '5·6번' };
...
export default function DeparturePage() {
  ...
  const { data: parkingData, isLoading: parkingLoading, refetch: parkingRefetch, isFetching: parkingFetching } = useParking(selectedTerminal);
  ...
  const computed = useMemo(() => {
    ...
    const currentGates = gateKeys
      .map((key) => ({ key, label: getDepartureGateLabel(selectedTerminal, key), value: currentData?.departure[key] ?? 0 }))
      .filter((g) => g.value > 0 || g.label !== '-');

    const heatmapRows = gateKeys
      .filter((key) => forecast.inOutData.some((d) => d.departure[key] > 0))
      .map((key) => ({ label: getDepartureGateLabel(selectedTerminal, key), data: forecast.inOutData.map((d) => ({ hour: d.hour, value: d.departure[key] })) }));

    const routeTotals = ROUTE_KEYS.map((key) => ({
      key,
      ...ROUTE_META[key],
      total: forecast.routeData.reduce((sum, d) => sum + ((d[key as keyof typeof d] as number) || 0), 0),
